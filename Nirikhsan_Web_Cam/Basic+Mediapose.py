...
class PoseApp:
    def __init__(self, window_title="Pose Guard (Multi-Target)"):
        ...
        self.sidebar_frame = ctk.CTkFrame(self.root, fg_color="#1a1a1a", width=self.sidebar_width)
        self.sidebar_frame.grid(row=0, column=1, sticky="nsew", padx=0, pady=0)
        self.sidebar_frame.grid_propagate(False)
        
        # Sidebar Header with Toggle Button
        sidebar_header = ctk.CTkFrame(self.sidebar_frame, fg_color="#0d0d0d", height=40)
        ...
        self.toggle_sidebar_btn = ctk.CTkButton(sidebar_header, text="◄ Hide", command=self.toggle_sidebar, 
                                                 width=40, height=30, fg_color="#27ae60", font=('Roboto', 9, 'bold'))
        self.toggle_sidebar_btn.pack(side="left", padx=5, pady=5)
        
        ctk.CTkLabel(sidebar_header, text="CONTROLS", font=("Roboto", 10, "bold"), text_color="gray").pack(side="left", padx=10, pady=5)
        
        # Scrollable content area in sidebar
        self.sidebar_scroll = ctk.CTkScrollableFrame(self.sidebar_frame, fg_color="#1a1a1a")
        ...
        ctk.CTkLabel(self.grp_camera, text="SYSTEM CONTROLS", font=("Roboto", 10, "bold"), text_color="gray").pack(anchor="w")
        
        self.cam_btns_frame = ctk.CTkFrame(self.grp_camera, fg_color="transparent")
        self.cam_btns_frame.pack(fill="x")
        
        self.btn_start = ctk.CTkButton(self.cam_btns_frame, text="▶ Start", command=self.start_camera, width=60, fg_color="#27ae60", font=btn_font)
        self.btn_start.pack(side="left", padx=2, fill="x", expand=True)
        
        self.btn_stop = ctk.CTkButton(self.cam_btns_frame, text="⏹ Stop", command=self.stop_camera, width=60, fg_color="#c0392b", font=btn_font)
        self.btn_stop.pack(side="left", padx=2, fill="x", expand=True)
        
        self.btn_snap = ctk.CTkButton(self.cam_btns_frame, text="📸 Snap", command=self.snap_photo, width=60, fg_color="#d35400", font=btn_font)
        self.btn_snap.pack(side="left", padx=2, fill="x", expand=True)
        
        self.btn_exit = ctk.CTkButton(self.cam_btns_frame, text="🚪 Exit", command=self.graceful_exit, width=50, fg_color="#34495e", font=btn_font)
        self.btn_exit.pack(side="left", padx=2, fill="x", expand=True)
        ...
        ctk.CTkLabel(self.grp_guard, text="GUARD MANAGEMENT", font=("Roboto", 10, "bold"), text_color="gray").pack(anchor="w")
        
        self.guard_btns_frame = ctk.CTkFrame(self.grp_guard, fg_color="transparent")
        self.guard_btns_frame.pack(fill="x")
        
        self.btn_add_guard = ctk.CTkButton(self.guard_btns_frame, text="➕ Add", command=self.add_guard_dialog, width=60, fg_color="#8e44ad", font=btn_font)
        self.btn_add_guard.pack(side="left", padx=2, fill="x", expand=True)
        
        self.btn_remove_guard = ctk.CTkButton(self.guard_btns_frame, text="❌ Remove", command=self.remove_guard_dialog, width=60, fg_color="#e74c3c", font=btn_font)
        self.btn_remove_guard.pack(side="left", padx=2, fill="x", expand=True)
        
        self.btn_refresh = ctk.CTkButton(self.guard_btns_frame, text="🔄 Refresh", command=self.load_targets, width=60, fg_color="#e67e22", font=btn_font)
        self.btn_refresh.pack(side="left", padx=2, fill="x", expand=True)

        # --- Group 3: Modes ---
        self.grp_modes = ctk.CTkFrame(self.sidebar_scroll, fg_color="transparent")
        self.grp_modes.pack(fill="x", padx=5, pady=5)
        ctk.CTkLabel(self.grp_modes, text="DETECTION MODES", font=("Roboto", 10, "bold"), text_color="gray").pack(anchor="w")
        
        self.mode_btns_frame = ctk.CTkFrame(self.grp_modes, fg_color="transparent")
        self.mode_btns_frame.pack(fill="x")
        
        self.btn_toggle_alert = ctk.CTkButton(self.mode_btns_frame, text="🔔 Alert", command=self.toggle_alert_mode, width=70, fg_color="#e67e22", font=btn_font)
        self.btn_toggle_alert.pack(side="left", padx=2, fill="x", expand=True)
        
        self.btn_fugitive = ctk.CTkButton(self.mode_btns_frame, text="🚨 Fugitive", command=self.toggle_fugitive_mode, width=70, fg_color="#8b0000", font=btn_font)
        self.btn_fugitive.pack(side="left", padx=2, fill="x", expand=True)
        
        self.btn_pro_detection = ctk.CTkButton(self.mode_btns_frame, text="🎯 Pro", command=self.toggle_pro_detection_mode, width=70, fg_color="#004a7f", font=btn_font)
        self.btn_pro_detection.pack(side="left", padx=2, fill="x", expand=True)
        ...
        ctk.CTkLabel(self.grp_settings, text="SETTINGS & ACTIONS", font=("Roboto", 10, "bold"), text_color="gray").pack(anchor="w")
        
        self.settings_grid = ctk.CTkFrame(self.grp_settings, fg_color="transparent")
        self.settings_grid.pack(fill="x")
        
        # Row 1: Interval & Action
        self.btn_set_interval = ctk.CTkButton(self.settings_grid, text="⏱ Interval", command=self.set_alert_interval_advanced, width=80, fg_color="#7f8c8d", font=btn_font)
        self.btn_set_interval.grid(row=0, column=0, padx=2, pady=2, sticky="ew")
        
        self.required_action_var = tk.StringVar(self.root)
        self.required_action_var.set("Hands Up")
        self.action_dropdown = ctk.CTkOptionMenu(self.settings_grid, values=["Hands Up", "Hands Crossed", 
                                            "One Hand Raised (Left)", "One Hand Raised (Right)", 
                                            "T-Pose", "Sit", "Standing"], command=self.on_action_change, fg_color="#3498db", text_color="white", font=btn_font_small)
        self.action_dropdown.grid(row=0, column=1, padx=2, pady=2, sticky="ew")
        
        # Row 2: Select Targets & Track
        self.btn_select_targets = ctk.CTkButton(self.settings_grid, text="📋 Select", command=self.open_target_selection_dialog, width=80, fg_color="#16a085", font=btn_font)
        self.btn_select_targets.grid(row=1, column=0, padx=2, pady=2, sticky="ew")
        
        self.btn_apply_targets = ctk.CTkButton(self.settings_grid, text="🎬 Track", command=self.apply_target_selection, width=80, fg_color="#16a085", font=btn_font)
        self.btn_apply_targets.grid(row=1, column=1, padx=2, pady=2, sticky="ew")
        ...
        ctk.CTkLabel(self.grp_preview, text="👤 TRACKED PERSONS", font=("Roboto", 10, "bold"), text_color="gray").pack(anchor="w")
        
        # Selected targets thumbnail display frame
        self.selected_thumbnails_frame = ctk.CTkFrame(self.grp_preview, fg_color="black", border_width=2)
        self.selected_thumbnails_frame.pack(fill="both", expand=True, padx=0, pady=2)
        ctk.CTkLabel(self.selected_thumbnails_frame, text="📋 Selected Targets:", text_color="white", font=("Arial", 8, "bold")).pack(anchor="w", padx=2, pady=2)
        self.selected_thumbnails_display = ctk.CTkFrame(self.selected_thumbnails_frame, fg_color="black")
        self.selected_thumbnails_display.pack(anchor="w", fill="both", expand=True, padx=5, pady=2)
        
        # Guard Preview (left)
        self.guard_preview_frame = ctk.CTkFrame(self.grp_preview, fg_color="darkgreen", border_width=2)
        self.guard_preview_frame.pack(fill="x", padx=0, pady=2)
        ctk.CTkLabel(self.guard_preview_frame, text="👮 GUARD", text_color="white", 
                font=("Arial", 9, "bold")).pack(fill="x", padx=1, pady=1)
        self.guard_preview_label = ctk.CTkLabel(self.guard_preview_frame, text="No Guard Selected", text_color="white", 
                                            font=("Arial", 8))
        self.guard_preview_label.pack(fill="x", padx=1, pady=1)
        
        # Fugitive Preview (right) - Initially hidden
        self.fugitive_preview_frame = ctk.CTkFrame(self.grp_preview, fg_color="darkred", border_width=2)
        ctk.CTkLabel(self.fugitive_preview_frame, text="🚨 FUGITIVE", text_color="white", 
                font=("Arial", 9, "bold")).pack(fill="x", padx=1, pady=1)
        self.fugitive_preview_label = ctk.CTkLabel(self.fugitive_preview_frame, text="No Fugitive", text_color="white", 
                                               font=("Arial", 8))
        self.fugitive_preview_label.pack(fill="x", padx=1, pady=1)
        
        # Status label at bottom
        self.status_label = ctk.CTkLabel(self.sidebar_scroll, text="FPS: 0 | MEM: 0 MB", text_color="white", font=('Roboto', 9))
    ...
    def update_selected_preview(self):
        """Update guard preview and show thumbnails of all selected targets"""
        # Clear previous thumbnails
        for widget in self.selected_thumbnails_display.winfo_children():
            widget.destroy()
        ...
        # Display thumbnails for all selected targets horizontally
            ...
            if filename:
                try:
                    ...
                    if img is not None:
                        ...
                        thumb_frame = tk.Frame(self.selected_thumbnails_display, bg="black")
                        thumb_frame.pack(side="left", padx=2)
                        
                        # Thumbnail image
                        thumb_label = tk.Label(thumb_frame, image=imgtk, bg="black")
                        thumb_label.pack()
                        
                        # Store photo reference to prevent garbage collection
                        self.photo_storage[f"thumb_{name}"] = imgtk
                        
                        # Name label
                        name_label = tk.Label(thumb_frame, text=name, bg="black", fg="yellow", 
                                            font=("Arial", 7), wraplength=50)
                        name_label.pack()
                except Exception:
                    pass
    ...
    def set_alert_interval_advanced(self):
        """Set alert interval with hours, minutes, seconds"""
        dialog = ctk.CTkToplevel(self.root)
        dialog.title("Set Alert Interval")
        dialog.geometry("350x200")
        dialog.grab_set()
        
        ctk.CTkLabel(dialog, text="Alert Interval (H:M:S)", font=("Roboto", 14, "bold")).pack(pady=10)
        
        frame = ctk.CTkFrame(dialog)
        frame.pack(pady=10, padx=20, fill="x")
        
        # Hours
        ctk.CTkLabel(frame, text="Hours:", font=("Roboto", 10)).grid(row=0, column=0, padx=5)
        h_var = ctk.StringVar(value="0")
        ctk.CTkEntry(frame, textvariable=h_var, width=60).grid(row=0, column=1, padx=5)
        
        # Minutes
        ctk.CTkLabel(frame, text="Minutes:", font=("Roboto", 10)).grid(row=0, column=2, padx=5)
        m_var = ctk.StringVar(value="0")
        ctk.CTkEntry(frame, textvariable=m_var, width=60).grid(row=0, column=3, padx=5)
        
        # Seconds
        ctk.CTkLabel(frame, text="Seconds:", font=("Roboto", 10)).grid(row=1, column=0, padx=5, pady=10)
        s_var = ctk.StringVar(value=str(self.alert_interval))
        ctk.CTkEntry(frame, textvariable=s_var, width=60).grid(row=1, column=1, padx=5, pady=10)
        ...
        def confirm():
            try:
                ...
                if total_seconds > 0:
                    ...
                    self.btn_set_interval.configure(text=f"⏱ Interval ({total_seconds}s)")
                    messagebox.showinfo("Success", f"Alert interval set to {total_seconds} seconds.")
                    dialog.destroy()
                else:
        ...
        ctk.CTkButton(dialog, text="Set", command=confirm, fg_color="green").pack(pady=15, padx=20, fill="x")
    ...
    def log_action_performed(self, guard_name, action, image_path, confidence):
        """Log when a guard performs the required action"""
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        self.temp_log.append((timestamp, guard_name, action, "Action Performed", image_path, f"{confidence:.2f}"))
        self.temp_log_counter += 1
        logger.info(f"✓ {guard_name}: {action} performed (conf: {confidence:.2f})")
        
    def log_action_not_performed(self, guard_name, expected_action, image_path, confidence):
        """Log when a guard does NOT perform the required action during interval"""
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        self.temp_log.append((timestamp, guard_name, expected_action, "Action Not Performed", image_path, f"{confidence:.2f}"))
        self.temp_log_counter += 1
        logger.warning(f"✗ {guard_name}: {expected_action} NOT performed (conf: {confidence:.2f})")
        
    def log_guard_missing(self, guard_name, image_path="N/A"):
        """Log when a guard is missing from the frame"""
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        self.temp_log.append((timestamp, guard_name, "N/A", "Guard Missing", image_path, "0.00"))
        self.temp_log_counter += 1
        logger.warning(f"⚠ {guard_name}: MISSING from frame")
