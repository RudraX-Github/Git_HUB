...
class PoseApp:
    def __init__(self, window_title="Pose Detection App"):
        ...
        self.tracked_box = None # (x1, y1, x2, y2) of the FULL BODY
        self.re_detect_counter = 0    
        self.RE_DETECT_INTERVAL = 30  # Re-detect every 30 frames
        self.last_known_confidence = 0.0
        self.RESIZE_SCALE = 0.5 # 320x240 for detection

        # --- NEW: Optimized Logging ---
        self.temp_log = [] # In-memory log
        ...
        try:
            ...
            options = vision.PoseLandmarkerOptions(
                ...
                # Must be >1 to find the target in the detection phase
                num_poses=5, 
                min_pose_detection_confidence=0.5,
                min_tracking_confidence=0.5)
            
            # --- We now need TWO landmarker instances ---
            # 1. For full-frame detection (slower)
            self.pose_landmarker_full = vision.PoseLandmarker.create_from_options(options)
            
            # 2. For cropped-frame tracking (faster, num_poses=1)
            options.num_poses = 1
            self.pose_landmarker_crop = vision.PoseLandmarker.create_from_options(options)
            
            print("Pose Landmarkers loaded successfully.")
        except Exception as e:
    ...
    # --- UPDATED: toggle_logging ---
    def toggle_logging(self):
        self.is_logging = not self.is_logging
        self.last_logged_action = None
        
        if self.is_logging:
            # --- NEW: Just clear the temp log ---
            self.temp_log.clear()
            self.btn_toggle_log.config(text="Stop Logging", bg="#E65100")
            print("[INFO] Logging Started (to temp file).")
        else:
            # --- NEW: Save log to file on stop ---
            self.btn_toggle_log.config(text="Start Logging", bg="#2196F3")
            print("[INFO] Logging Stopped.")
            self.save_log_to_file()
            
    # --- NEW: Save Log Function ---
    def save_log_to_file(self):
        """Appends the contents of the temp log to the permanent CSV file."""
        if not self.temp_log:
            print("[INFO] No log entries to save.")
            return
            
        print(f"[INFO] Saving {len(self.temp_log)} log entries to {csv_file}...")
        try:
    ...
    def process_tracking_frame_optimized(self, frame):
        ...
        # --- Convert frame to RGB once ---
        rgb_full_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        
        # -----------------------------------------------
        # --- PART 1: RUN FULL-FRAME POSE DETECTION ---
        # -----------------------------------------------
        # This is now the first step, as we need the poses to find the body.
        
        mp_image = mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb_full_frame)
        self.frame_timestamp_ms = int(time.time() * 1000)
        try:
            pose_results = self.pose_landmarker_full.detect_for_video(mp_image, self.frame_timestamp_ms)
        except Exception as e:
            print(f"Error during pose detection: {e}")
            pose_results = None 
        
        # -----------------------------------------------
        # --- PART 2: FIND/TRACK THE TARGET'S FACE ---
        # -----------------------------------------------
        
        # --- PATH 2.A: WE ARE ACTIVELY TRACKING (FAST) ---
        if self.is_tracking:
            success, box = self.tracker.update(frame)
            if success:
                target_visible = True 
                x, y, w, h = [int(v) for v in box]
                # We store the BODY box, not the face box
                self.tracked_box = (x, y, x + w, y + h) # (x1, y1, x2, y2)
            else:
                # Tracker failed, force re-detection
                self.is_tracking = False
                self.tracker = None
        
        # --- PATH 2.B: WE ARE DETECTING (SLOWER) ---
        else:
            if self.target_face_encoding is not None:
                rgb_small_frame = cv2.resize(rgb_full_frame, (0, 0), fx=self.RESIZE_SCALE, fy=self.RESIZE_SCALE)
                ...
                if face_locations:
                    ...
                    if best_match_distance < 0.55: 
                        target_visible = True
                        print("[INFO] Target face found. Linking to pose...")
                        (top, right, bottom, left) = face_locations[best_match_index]
                        
                        scale_up = 1 / self.RESIZE_SCALE
                        face_x1 = int(left * scale_up)
                        face_y1 = int(top * scale_up)
                        face_x2 = int(right * scale_up)
                        face_y2 = int(bottom * scale_up)
                        
                        self.last_known_confidence = (1.0 - best_match_distance) * 100
                        
                        # --- LINK FACE TO POSE to find FULL BODY ---
                        if pose_results and pose_results.pose_landmarks:
                            for pose_landmarks_list in pose_results.pose_landmarks:
                                nose = pose_landmarks_list[0]
                                nose_x = int(nose.x * self.frame_w)
                                nose_y = int(nose.y * self.frame_h)

                                if (nose_x > face_x1 and nose_x < face_x2 and nose_y > face_y1 and nose_y < face_y2):
                                    print("[INFO] Pose linked. Initializing BODY tracker.")
                                    # Calculate a body box from all landmarks
                                    min_x = min(lm.x for lm in pose_landmarks_list)
                                    max_x = max(lm.x for lm in pose_landmarks_list)
                                    min_y = min(lm.y for lm in pose_landmarks_list)
                                    max_y = max(lm.y for lm in pose_landmarks_list)
                                    
                                    pad = 20 # Padding
                                    x1 = max(0, int(min_x * self.frame_w) - pad)
                                    y1 = max(0, int(min_y * self.frame_h) - pad)
                                    x2 = min(self.frame_w, int(max_x * self.frame_w) + pad)
                                    y2 = min(self.frame_h, int(max_y * self.frame_h) + pad)
                                    ...
                                    try:
                                        ...
                                        print(f"--- ERROR: {e} ---")
                                        self.stop_camera()
                                    break # Stop looping poses

        # -----------------------------------------------
        # --- PART 3: DRAWING & LOGGING (NOW DECOUPLED) ---
        # -----------------------------------------------
        ...
        # --- Target IS visible ---
        (x1, y1, x2, y2) = self.tracked_box # This is the BODY box
        confidence_text = f"({self.last_known_confidence:.0f}%)"
        
        # Draw the body box
        cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
        
        # --- We now run pose on the CROP of the body box ---
        pad = 5 # Small padding
        crop_x1 = max(0, x1 - pad)
        crop_y1 = max(0, y1 - pad)
        crop_x2 = min(self.frame_w, x2 + pad)
        crop_y2 = min(self.frame_h, y2 + pad)
        ...
        mp_image_crop = mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb_crop)
        
        try:
            # Use the FASTER (1-pose) landmarker
            results_crop = self.pose_landmarker_crop.detect_for_video(mp_image_crop, self.frame_timestamp_ms)
        except Exception as e:
            print(f"Error during cropped pose detection: {e}")
            results_crop = None 
        
        if results_crop and results_crop.pose_landmarks:
            pose_landmarks_list = results_crop.pose_landmarks[0]
            pose_landmarks_proto = landmark_pb2.NormalizedLandmarkList()
            
            nose = pose_landmarks_list[0]
            nose_x_rel = int(nose.x * (crop_x2 - crop_x1)) + crop_x1
            nose_y_rel = int(nose.y * (crop_y2 - crop_y1)) + crop_y1

            for landmark_data in pose_landmarks_list:
                landmark_proto = pose_landmarks_proto.landmark.add()
                # Offset landmarks from crop to full frame
                landmark_proto.x = (landmark_data.x * (crop_x2 - crop_x1) + crop_x1) / self.frame_w
                landmark_proto.y = (landmark_data.y * (crop_y2 - crop_y1) + crop_y1) / self.frame_h
            ...
            current_action = classify_action(pose_landmarks_list, (crop_y2-crop_y1), (crop_x2-crop_x1))
